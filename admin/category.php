                            ...
                            <div class="card">
                                ...
                                <div class="card-body">
                                    ...
                                    if (empty($_POST['search']) and empty($_COOKIE['search'])) {
                                        ...
                                        $statement = $pdo->prepare("SELECT * FROM categories ORDER BY id DESC LIMIT $offset,$numOfRecord");
                                        $statement->execute();
                                        ...
                                        $numOfRecord = 6;
                                        $offset = ($pageno - 1) * $numOfRecord;
                                        $statement = $pdo->prepare("SELECT * FROM categories WHERE name LIKE '%$search%' ORDER BY id DESC");
                                        $statement->execute();
                                        $rawResult = $statement->fetchAll();

                                        $total_pages = ceil(count($rawResult) / $numOfRecord);
                                        $statement = $pdo->prepare("SELECT * FROM categories WHERE name LIKE '%$search%' ORDER BY id DESC LIMIT $offset,$numOfRecord");
                                        $statement->execute();
                                        $result = $statement->fetchAll();
                                    }
                                    ?>
                                        ...
                                        <tbody>
                                            ...
                                            if ($result) : foreach ($result as $value) : ?>
                                                    <tr>
                                                        ...
                                                        <td>
                                                            <div>
                                                                <?= escape(substr($value['description'], 0, 150)) ?>
                                                            </div>
                                                        </td>
                                                        <td>
                                                            <a href="cat_edit.php?id=<?= $value['id'] ?>" class="btn btn-warning" type="button">Edit</a>
                                                            <a href="cat_delete.php?id=<?= $value['id'] ?>" class="btn btn-danger" onclick="return confirm('Are you sure to delete this Category');" type="button">Delete</a>
                                                        </td>
                                                    </tr>
                                            <?php $i++;
                                                endforeach;
