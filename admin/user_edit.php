...
if ($_POST) {
    ...
    if (empty($_POST['password'])) {
        ...
        if (empty($name) or empty($email) or empty($address) or empty($phone)) {
            if (empty($name)) {
            ...
            if ($user) {
                ...
                $statement = $pdo->prepare("UPDATE users SET name=:name,email=:email,password=:password,address=:address,phone=:phone,role=:role WHERE id=:id");
                $result = $statement->execute([":name" => $name, ":email" => $email, ":password" => $password, ":address" => $address, ":phone" => $phone, ":role" => $role, "id" => $id]);
                if ($result) {
        ...
        if (empty($name) or empty($email) or strLen($password) < 4 or empty($address) or empty($phone)) {
            if (empty($name)) {
                $nameError = "Username is required";
            }
            if (empty($email)) {
                                ...
                                <div class="card-body">
                                    <form action="" method="post">
                                        ...
                                        <div class="form-group">
                                            ...
                                            <p class="text-danger d-inline-block ml-2"><?= empty($addressError) ? "" : "*" . $addressError ?></p>
                                            <input type="text" class="form-control" id="address" value="<?= escape($result[0]['address']) ?>" name="address">
                                        </div>
                                        <div class="form-group">
                                            <label for="name">Phone</label>
                                            <p class="text-danger d-inline-block ml-2"><?= empty($phoneError) ? "" : "*" . $phoneError ?></p>
                                            <input type="number" class="form-control" id="phone" value="<?= escape($result[0]['phone']) ?>" name="phone">
                                        </div>
                                        <div class="form-group">
                                            <label>Role</label>
